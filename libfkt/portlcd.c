...
** 
******************************************************************************/
void lcd_write_4bit( unsigned long c ) 
{
  IO1DIR |= LCD_DATA | LCD_CTRL;
  IO1CLR  = LCD_RW   | LCD_DATA;
  IO1SET  = (c & 0xF) << LCD_DATA_POSITION;            //-- shift data to io-bits
  IO1SET  = LCD_E;
  lcd_delay (10);
  IO1CLR  = LCD_E;
  lcd_delay (10);
...
** 
******************************************************************************/
void lcd_write( unsigned long c ) 
{
  lcd_write_4bit (c >> 4);
  lcd_write_4bit (c);
  return;
...
** 
******************************************************************************/
unsigned long lcd_read_stat( void ) 
{
  unsigned long stat;
  ...
  IO1SET  = LCD_E;
  lcd_delay (10);
  stat    = (IO1PIN >> LCD_STATUS_MSB) & 0xF0;
  IO1CLR  = LCD_E;
  lcd_delay (10);
  IO1SET  = LCD_E;
  lcd_delay (10);
  stat   |= (IO1PIN >> LCD_STATUS_LSB) & 0xF;
  IO1CLR  = LCD_E;
  return (stat);
}
...
** 
******************************************************************************/
void lcd_write_cmd( unsigned long c ) 
{
  lcd_wait_busy();
  IO1CLR = LCD_RS;
  lcd_write(c);
...
** 
******************************************************************************/
void lcd_write_data( unsigned long d ) 
{
  lcd_wait_busy();
  IO1SET = LCD_RS;
  lcd_write(d);
...
//#endif
  ...
  LCD_load( (char *)&UserFont, sizeof (UserFont) );
  LCD_cls();
  return;
}
...
** 
******************************************************************************/
void LCD_load( char *fp, unsigned long cnt ) 
{
  unsigned long i;
...
** 
******************************************************************************/
void LCD_gotoxy( unsigned long x, unsigned long y ) 
{
  unsigned long c;
...
** 
******************************************************************************/
void LCD_bargraph( unsigned long val, unsigned long size ) 
{
  unsigned long i;
